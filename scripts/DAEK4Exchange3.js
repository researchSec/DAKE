...
async function main() {
  ...
  exchangeContract[0] = await new ethers.Contract("0xEF73E0CA4cd8aFea6D9Db4699131ba2ce89906D6",abi,addr[0]);
  ...
  const h1pi = await eP.multiplyAndPow(gvpi,pkQ,sk,uSk);
  const gvu = await eP.powSecret(gvpi,uSk);
  const hashforkey1 =  crypto.createHash('sha256');
  hashforkey1.update(h1pi + gvu + pk + uPk + pkQ + gvpi + idP + idQpi);
