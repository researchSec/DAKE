...
async function start() {
  ...
  const contractObj = chainsql.contract(JSON.parse(abi),"z98KH1xiWUxeZj66gMP8f4V52tXRuWuJNq");
  ...
  ins = await contractObj.methods.Initialization(users[0].address,idP,u).submit({
    Gas: 8000000,
  ...
  ins = await contractObj.methods.setStrforCommunicator(users[1].address,c1).submit({
      Gas: 8000000,
      expect: "validate_success"
    }).then(res => {
        console.log(res);
  ...
  timecost = Date.now() - begin;
  console.log(`time for Q at stage 1 = ${timecost} MS`);
  ...
  const c1pi = await contractObj.methods.getStrfromCommunicator(users[1].address).call();
  ...
  ins = await contractObj.methods.setStrforCommunicator(users[0].address,c2).submit({
    Gas: 8000000,
    expect: "validate_success"
  }).then(res => {
      console.log(res);
  ...
  timecost = Date.now() - begin;
  console.log(`time for P at stage 2 = ${timecost} MS`);
  ...
  begin = Date.now();
  //for Q
  const c2pi = await contractObj.methods.getStrfromCommunicator(users[0].address).call();
  ...
  timecost = Date.now() - begin;
  console.log(`time for Q at stage 3 = ${timecost} MS`);
