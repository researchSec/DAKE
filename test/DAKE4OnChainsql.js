...
async function start() {
  ...
  const contractObj = chainsql.contract(JSON.parse(abi),"zU9m6nuXhCJSkJzdruMBqB8AgPWicwuUPY");
  ...
  const gu = exKeysP.pk;
  ins = await contractObj.methods.setRforCommunicator(users[0].address,users[1].address,gu).submit({
    Gas: 8000000,
    expect: "validate_success"
  }).then(res => {
      console.log(res);
  ...
  const gupi = await contractObj.methods.getRfromCommunicator(users[1].address,users[0].address).call();
  ...
  //send gv , k1Q to P
  ins = await contractObj.methods.setRforCommunicator(users[1].address,users[0].address,gv).submit({
    Gas: 8000000,
    expect: "validate_success"
  }).then(res => {
      console.log(res);
  ...
  ins = await contractObj.methods.setSigforCommunicator(users[1].address,users[0].address,k1Q).submit({
    Gas: 8000000,
    expect: "validate_success"
  }).then(res => {
      console.log(res);
  ...
  const gvpi =  await contractObj.methods.getRfromCommunicator(users[0].address,users[1].address).call();
  const K1Qpi =  await contractObj.methods.getsigfromCommunicator(users[0].address,users[1].address).call();
  ...
  //send k2P to Q
  ins = await contractObj.methods.setSigforCommunicator(users[0].address,users[1].address,k2P).submit({
    Gas: 8000000,
    expect: "validate_success"
  }).then(res => {
      console.log(res);
  ...
  //for Q
  const K2Ppi = await contractObj.methods.getsigfromCommunicator(users[1].address,users[0].address).call();
