...
async function start() {
  ...
  const contractObj = chainsql.contract(JSON.parse(abi),"zUpYpQCv7qGACC4YxiTiJ3VyB1V3BHiiPX");
  ...
  ins = await contractObj.methods.setRforCommunicator(users[0].address,users[1].address,u).submit({
    Gas: 8000000,
    expect: "validate_success"
  }).then(res => {
      console.log(res);
  ...
  ins = await contractObj.methods.setSigforCommunicator(users[0].address,users[1].address,transdelta).submit({
    Gas: 8000000,
    expect: "validate_success"
  }).then(res => {
      console.log(res);
  ...
  begin = Date.now();
  //for Q
  const sig = await contractObj.methods.getsigfromCommunicator(users[1].address,users[0].address).call();
  const upi = await contractObj.methods.getRfromCommunicator(users[1].address,users[0].address).call();
  ...
  ins = await contractObj.methods.setSigforCommunicator(users[1].address,users[0].address,transdelta2).submit({
    Gas: 8000000,
    expect: "validate_success"
  }).then(res => {
      console.log(res);
  ...
  const pkQ = ins[1];
  const idQpi = ins[2];
  if(!(addrQ === users[1].address)){console.log("error3"); return;}
  const sig2 = await contractObj.methods.getsigfromCommunicator(users[0].address,users[1].address).call();
